...
export default function BottomNav() {
  ...
  const myStoreActive = active(['/my-store', '/my-listings', '/wallet', '/history', '/watermark']);
  const profileActive = active(['/profile', '/dashboard', '/send', '/notifications', '/verify', '/referrals', '/login', '/register', '/escrow']);
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 md:hidden bg-[#1B4332] border-t border-white/10 flex safe-area-bottom">
      ...
      {/* Community */}
      <Link href="/my-store" className={`flex-1 flex flex-col items-center py-2.5 gap-0.5 min-h-[56px] ${clr(myStoreActive)}`}>
        <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
