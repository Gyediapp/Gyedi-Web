        ...
        return (
          <Link
            ...
            className={`flex-1 flex flex-col items-center py-2 gap-0.5 transition-colors ${
              active ? 'text-[#F5A623]' : 'text-white/40'
            }`}
          >
            <Icon active={active} />
...
function HomeIcon({ active }: { active: boolean }) {
  return (
    <svg className="w-5 h-5" fill={active ? '#F5A623' : 'none'} stroke={active ? '#F5A623' : 'currentColor'} strokeWidth={2} viewBox="0 0 24 24">
...
function MarketIcon({ active }: { active: boolean }) {
  return (
    <svg className="w-5 h-5" fill="none" stroke={active ? '#F5A623' : 'currentColor'} strokeWidth={2} viewBox="0 0 24 24">
...
function EscrowIcon({ active }: { active: boolean }) {
  return (
    <div className={`w-10 h-10 rounded-full flex items-center justify-center -mt-5 shadow-lg ${active ? 'bg-[#F5A623] ring-4 ring-[#F5A623]/30' : 'bg-[#F5A623]/80'}`}>
