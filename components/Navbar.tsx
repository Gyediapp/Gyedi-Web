...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    if (token) {
      fetch(`${process.env.NEXT_PUBLIC_API_URL ?? 'https://gyedi-api-production.up.railway.app/api'}/notifications/unread-count`, {
        headers: { Authorization: `Bearer ${token}` },
      }).then(r => r.json()).then(d => { if (d.count) setUnreadCount(d.count); }).catch(() => {});
    }
  }, []);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3 h-14">
            ...
            <Link href="/" className="flex items-center gap-2 flex-shrink-0">
              ...
              <span className="text-white/40 text-[11px] font-medium hidden sm:block border-l border-white/20 pl-2">
                Secure Marketplace
              </span>
            </Link>
            ...
            <form onSubmit={handleSearch} className="hidden md:flex flex-1 max-w-2xl mx-4">
              <div className="relative w-full">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-24 py-2.5 bg-white rounded-xl text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#F5A623]/50 transition-all"
                />
                <button type="submit"
            ...
            <div className="flex items-center gap-1 ml-auto">
              ...
              <Link href="/favourites" className="p-2 text-white/60 hover:text-white transition-colors">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
              ...
              {/* Hamburger mobile */}
              <button onClick={() => setOpen(!open)} className="md:hidden p-2 text-white/70 hover:text-white transition-colors ml-1">
          ...
          <div className="flex items-center gap-1 h-10">
            {NAV_LINKS.map(link => (
              <Link key={link.href} href={link.href}
                className="px-4 py-1.5 text-white/65 hover:text-white hover:bg-white/10 rounded-lg text-sm font-medium transition-colors">
                {link.label}
              </Link>
            ))}
            <Link href="/referrals"
              className="px-4 py-1.5 text-[#F5A623] hover:text-[#F5A623]/80 hover:bg-white/10 rounded-lg text-sm font-bold transition-colors flex items-center gap-1">
              Refer &amp; Earn
            </Link>
          </div>
        </div>
