  ...
  return (
    <div className="min-h-screen bg-[#F4F6F8] pb-28">
      ...
      <div className="bg-[#1B4332] px-5 pt-12 pb-8">
        <h1 className="text-white font-black text-2xl">Gyedi Community</h1>
        <p className="text-green-300 text-sm mt-1">Learn, Share, Grow 🇬🇭</p>
        <div className="flex gap-3 mt-4">
          <a href="/community/forum"
            className="bg-[#F5A623] text-[#1B4332] font-bold text-sm px-4 py-2 rounded-xl hover:bg-[#D4881A] transition-colors">
            Browse Forum
          </a>
          <a href="/blog"
            className="bg-white/10 text-white font-bold text-sm px-4 py-2 rounded-xl hover:bg-white/20 transition-colors">
            Read Blog
          </a>
        </div>
      </div>
          ...
          <>
            ...
            <section>
              <div className="flex items-center justify-between mb-3">
                <h2 className="font-black text-gray-900 text-base">Forum</h2>
            ...
            {posts.length > 0 && (
              <section>
                <div className="flex items-center justify-between mb-3">
                  <h2 className="font-black text-gray-900 text-base">Recent Discussions</h2>
                  <a href="/community/forum" className="text-[#1B4332] text-xs font-bold hover:underline">See all →</a>
                </div>
            ...
            {blogPosts.length > 0 && (
              <section>
                <div className="flex items-center justify-between mb-3">
                  <h2 className="font-black text-gray-900 text-base">Latest from the Blog</h2>
                  ...
                  {blogPosts.map(bp => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {bp.excerpt && (
                          <p className="text-xs text-gray-400 mt-1 line-clamp-2">{bp.excerpt}</p>
                        )}
                        {bp.publishedAt && (
                          <p className="text-xs text-gray-300 mt-1">
                            {new Date(bp.publishedAt).toLocaleDateString('en-GH', { day: 'numeric', month: 'short', year: 'numeric' })}
                          </p>
                        )}
                      </div>
                    </a>
