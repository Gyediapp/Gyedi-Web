...
type Preview = { file: File; objectUrl: string; uploaded?: string };
...
export default function SellPage() {
  ...
  function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const toAdd = files.slice(0, remaining).map(f => ({ file: f, objectUrl: URL.createObjectURL(f) }));
  ...
  async function uploadPending(token: string): Promise<string[]> {
    const pending = previews.filter(p => !p.uploaded);
    if (!pending.length) return previews.map(p => p.uploaded!).filter(Boolean);
    ...
    setPreviews(prev => {
      let urlIdx = 0;
      return prev.map(p => p.uploaded ? p : { ...p, uploaded: urls[urlIdx++] });
    });
    return [
      ...previews.filter(p => p.uploaded).map(p => p.uploaded!),
      ...urls,
    ];
  }
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      imageUrls = await uploadPending(token);
    } catch (err: unknown) {
    ...
    try {
      ...
      setSuccess('Listing published! It\'s now live on the marketplace.');
      setPreviews([]);
    ...
    return (
      <div className="min-h-screen bg-[#F4F6F8] flex items-center justify-center py-12 px-4">
        <div className="text-center max-w-sm">
          ...
          <Link href="/login" className="block bg-[#1B4332] hover:bg-[#0F2B1F] text-white font-bold px-8 py-3.5 rounded-xl transition-colors">
    ...
    <div className="min-h-screen bg-[#F4F6F8] py-10">
      <div className="max-w-2xl mx-auto px-4 sm:px-6">
        ...
        <div className="mb-7">
          ...
          <p className="text-gray-400 mt-1.5 text-sm">Your item will be live on the marketplace immediately</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sm:p-8 space-y-6">
          ...
          <div>
            <label className="block text-sm font-bold text-gray-700 mb-2">
              Photos <span className="text-gray-400 font-normal">(up to {MAX_IMAGES}, first is the main photo)</span>
            </label>
            ...
            {previews.length > 0 && (
              <div className="grid grid-cols-3 sm:grid-cols-5 gap-2 mb-3">
                {previews.map((p, i) => (
                  <div key={i} className="relative aspect-square rounded-xl overflow-hidden border-2 border-gray-200 bg-gray-50 group">
                    <Image src={p.objectUrl} alt="" fill className="object-cover" unoptimized />
                    {i === 0 && (
                      <span className="absolute top-1 left-1 bg-[#F5A623] text-[#1B4332] text-[9px] font-black px-1.5 py-0.5 rounded-full">MAIN</span>
                    )}
                    {p.uploaded && (
                ...
                {previews.length < MAX_IMAGES && (
                  <button
                    ...
                    className="aspect-square rounded-xl border-2 border-dashed border-gray-300 flex flex-col items-center justify-center text-gray-400 hover:border-[#1B4332] hover:text-[#1B4332] transition-colors"
                  >
            ...
            {previews.length === 0 && (
              <button
                type="button"
                onClick={() => fileRef.current?.click()}
                className="w-full border-2 border-dashed border-gray-300 rounded-xl py-10 flex flex-col items-center gap-2 text-gray-400 hover:border-[#1B4332] hover:text-[#1B4332] transition-colors"
              >
                ...
                <div className="text-center">
                  ...
                  <p className="text-xs mt-0.5">JPG, PNG, WebP · Max 5 MB each</p>
                </div>
              </button>
            )}
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#1B4332]/30 focus:border-[#1B4332]"
            />
          </div>
          ...
          <div>
            ...
            <select
              name="category"
              required
              className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#1B4332]/30 focus:border-[#1B4332] bg-white"
            >
          ...
          <div>
            ...
            <div className="relative">
              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 text-sm font-bold">GHS</span>
              <input
                name="price"
                type="number"
                required
                ...
                step="0.01"
                placeholder="0.00"
                className="w-full pl-14 pr-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#1B4332]/30 focus:border-[#1B4332]"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#1B4332]/30 focus:border-[#1B4332] resize-none"
            />
          </div>
